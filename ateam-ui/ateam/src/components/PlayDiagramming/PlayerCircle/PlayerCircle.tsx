...
import { isLine } from "../Common/utilities";
    ...
    useEffect(() => {
        ...
        if (state.selectedId) {
            if (isLine(state.selectedId)) {
                const parentLine = playerCircleData.lines.get(state.selectedId);
                if (parentLine?.connector?.childId != null) {
        ...
        if (!lineAlreadyExists) {
            ...
            console.log("LINE ALREADY EXISTS, IGNORING DRAW LINE");
        }
